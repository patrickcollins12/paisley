...
export function TagEditorPopover({ values, allValues, onChange, cellValues, contentHeader, inputPlaceholder }) {
  ...
  useKey('Escape', closePopover);

  return (
    <Popover open={show}>
      <PopoverAnchor>
        <div className="min-h-[26px]" onClick={() => { setShow(!show) }}>
      ...
      {show &&
        <PopoverContent data- className='w-[450px] p-3 absolute -top-[53px] -left-[19px]' align='start'>
          {contentHeader || ""}

          <p>{`Show is: ${show}` && console.log('PopoverContent')}</p>
