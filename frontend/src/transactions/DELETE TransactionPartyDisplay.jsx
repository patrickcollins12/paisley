...
export function TransactionPartyDisplay({ data, onUpdate }) {
  const id = data.id;

  // both need to be valid arrays
  if (!(Array.isArray(data.auto_party) && Array.isArray(data.manual_party))) {
    return (<div>tag array retrieval error</div>)
  }

  const [autoCategorize, setAutoCategorize] = useState(data.auto_categorize);
  const [autoTags] = useState(data.auto_party);
  const [manualTags, setManualTags] = useState(data.manual_party);
  const [tags, setTags] = useState(data.party);

  // const displayedTags = autoCategorize ? [...new Set([...manualTags, ...autoTags])] : [...manualTags];
  const { data: allTags } = useFetchTags("parties"); // for the select dropdown
