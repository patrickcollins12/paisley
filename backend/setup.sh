...
# Step 0: Fail if config.js or transactions.db already exists
...
# Step 1: Create local Paisley config directory
mkdir -p "$PAISLEY_DIR"
mkdir -p "$BANK_STATEMENTS_DIR"
echo "📁 Created directory: $PAISLEY_DIR"

# Step 2: Copy config and users file
cp "$CONFIG_TEMPLATE" "$CONFIG_FILE"
echo "📄 Copied config.js to $CONFIG_FILE"

# Step 3: Create a new empty database from demo schema
